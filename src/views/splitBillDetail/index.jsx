...
import ReceiptPDF from '../splitBillStruck';
...
const SplitBillDetail = () => {
  ...
  const handleShare = () => {
    navigator.clipboard.writeText(
      `http://localhost:4173/split-bill/detail/shared/${id}`
    );
    Swal.fire({
      icon: 'success',
      title: 'Berhasil menyalin tautan Split Bill',
  ...
  const handleWhatsAppShare = () => {
    const shareText = `Split Bill: ${
      splitBillData.title
    }\nTotal: ${formatRupiah(
      splitBillData.totalAmount
    )}\nLink: http://localhost:4173/split-bill/detail/shared/${id}`;
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(shareText)}`;
    window.open(whatsappUrl, '_blank');
  };

  const handleEmailShare = () => {
    const subject = `Split Bill: ${splitBillData.title}`;
    const body = `Split Bill Details:\n\nTitle: ${
      splitBillData.title
    }\nTotal: ${formatRupiah(
      splitBillData.totalAmount
    )}\nLink: http://localhost:4173/split-bill/detail/shared/${id}`;
        ...
        <div className="bg-secondary p-4 border border-gray-300 rounded-lg shadow-lg h-fit w-full max-w-4xl">
          <div className="p-4 sm:p-6 bg-white shadow-lg rounded-lg">
            ...
            <div className="flex flex-col sm:flex-row justify-between mt-6">
              <button
                className="bg-gray-500 text-white p-3 rounded-lg hover:bg-gray-600 transition duration-200 mb-4 sm:mb-0"
                onClick={() => navigate('/main')}
              >
                Kembali
              </button>
              <div className="flex items-center justify-end gap-2">
                <ReceiptPDF splitBill={splitBillData} />
                <button
                  className="p-3 bg-gray-500 rounded-lg  hover:bg-gray-200 transition duration-200"
                ...
                <button
                  ...
                  onClick={handleShare}
                >
                  <img src={linkIcon} alt="Copy Link" className="h-5 w-5" />
                </button>
              </div>
            </div>
          </div>
